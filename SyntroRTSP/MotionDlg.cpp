...
void MotionDlg::onOk()
{
    ...
    if (m_minDelta->sliderPosition() != settings->value(SYNTRORTSP_MOTION_MIN_DELTA).toInt()) {
        settings->setValue(SYNTRORTSP_MOTION_MIN_DELTA, m_minDelta->sliderPosition());
		changed = true;
	}

    if (m_motionDelta->text() != settings->value(SYNTRORTSP_MOTION_DELTA_INTERVAL).toString()) {
        settings->setValue(SYNTRORTSP_MOTION_DELTA_INTERVAL, m_motionDelta->text());
		changed = true;
	}

    if (m_preroll->text() != settings->value(SYNTRORTSP_MOTION_PREROLL).toString()) {
        settings->setValue(SYNTRORTSP_MOTION_PREROLL, m_preroll->text());
		changed = true;
	}

    if (m_postroll->text() != settings->value(SYNTRORTSP_MOTION_POSTROLL).toString()) {
        settings->setValue(SYNTRORTSP_MOTION_POSTROLL, m_postroll->text());
		changed = true;
	}
...
void MotionDlg::layoutWindow()
{
    ...
    m_minDelta->setSliderPosition(settings->value(SYNTRORTSP_MOTION_MIN_DELTA).toInt());
    m_minDeltaValue->setText(settings->value(SYNTRORTSP_MOTION_MIN_DELTA).toString());
    ...
    m_motionDelta->setText(settings->value(SYNTRORTSP_MOTION_DELTA_INTERVAL).toString());
    ...
    m_preroll->setText(settings->value(SYNTRORTSP_MOTION_PREROLL).toString());
    ...
    m_postroll->setText(settings->value(SYNTRORTSP_MOTION_POSTROLL).toString());
